function [] = ps_select(reest_flag, plot_flag)
    ...
    fd = fopen('../selpsc.in');
    cand_coh_thresh = fscanf(fd, '%f', 1);
    fclose(fd);
    logit(sprintf('cand_coh_thresh = %.3f', cand_coh_thresh));
    ...
    logit(sprintf('%i PS candidates at start', n_ps));
    ...
    if plot_flag == 1
        ...
        histogram(D_A, 50, 'FaceColor', 'magenta', 'FaceAlpha', 0.4, 'EdgeAlpha', 0.4);

        xlabel('D_A');
        title(sprintf('Dispersion at %i PS candidate points', length(D_A)));

        xline(mean(D_A), '-', {'Mean'});
        xline(low_coh_thresh/100, '-', {'low\_coh\_thresh'});
        xline(cand_coh_thresh, '-', {'cand\_coh\_thresh'});
    ...
    if reest_flag == 3 % reestimate for all candidate pixels
        ...
        for i = 1:length(D_A_max) - 1
            ...
            if plot_flag == 1
                figure(2);
                plot(pm.coh_bins, Na, 'magenta', 'LineWidth', 2);
                hold on;
                plot(pm.coh_bins, Nr, 'black', 'LineWidth', 2);
                legend('Data', 'Random');
            ...
            if isempty(ok_ix)
                ...
                if min_fit_ix <= 0
                    ...
                    [p, ~, mu] = polyfit(percent_rand(min_fit_ix:max_fit_ix), [min_fit_ix * 0.01:0.01:max_fit_ix * 0.01], 3); % KS
    ...
    if plot_flag == 1
        figure(4);
        histogram(pm.coh_ps, 50, 'FaceColor', 'magenta', 'FaceAlpha', 0.4, 'EdgeAlpha', 0.4);
        % xline(coh_thresh, '-', {'coh\_thresh'});
        title('coh\_ps');
        drawnow;
    end    
    ...
    if gamma_stdev_reject > 0
        fprintf("Performing gamma_stdev_reject\n")

        ph_res_cpx = exp(1i * pm.ph_res(:, ifg_index));
    ...
    if reest_flag ~= 1
        ...
        for i = 1:length(D_A_max) - 1
            ...
            if plot_flag == 1
                figure(5);
                plot(pm.coh_bins, Na, 'cyan', 'LineWidth', 2);
                hold on;
                plot(pm.coh_bins, Nr, 'black', 'LineWidth', 2);
                legend('Data', 'Random');
    ...
    stamps_save(selectname, ix, keep_ix, ph_patch2, ph_res2, K_ps2, ...
        C_ps2, coh_ps2, coh_thresh, coh_thresh_coeffs, clap_alpha, ...
        clap_beta, n_win, max_percent_rand, gamma_stdev_reject, ...
