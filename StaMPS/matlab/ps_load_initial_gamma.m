function [] = ps_load_initial_gamma(endian)
    ...
    phname = './pscands.1.ph'; % for each PS cand, a complex for each ifg
    ...
    for i = 1:n_ifg
        [ph_bit, ~] = fread(fid, [(n_ps) * 2, 1], 'float=>single');
        ph(:, i) = complex(ph_bit(1:2:end), ph_bit(2:2:end));
    end

    for i = 1:n_ifg
        mn = mean(ph(:, i));
        fprintf("%s: mean(ph) = %+.3f%+.3fi\n", ifgs{i}, real(mn), imag(mn));
    end
    ...
    stamps_save(savename, ij, lonlat, xy, bperp, day, master_day, ...
        master_ix, n_ifg, n_image, n_ps, sort_ix, ll0, ...
        mean_incidence, mean_range);
