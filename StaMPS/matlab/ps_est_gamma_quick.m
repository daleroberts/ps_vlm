    ...
    if restart_flag<2
        ...
        for i=1:n_ps
            ...
            if sum(psdph==0)==0  % insist on a non-null value in every ifg
                [Kopt,Copt,cohopt,ph_residual]=ps_topofit(psdph,bp.bperp_mat(i,:)',n_trial_wraps,doplots);
                K_ps(i)=Kopt(1);
                C_ps(i)=Copt(1);
                coh_ps(i)=cohopt(1);
                N_opt(i)=length(Kopt);
        ...
        coh_ps(isnan(coh_ps)) = 0; % DR nan

        step_number=1;

        if true
           figure(1)
           title("weighting and coh")
           subplot(2,1,1)
           hist(weighting,100)
           subplot(2,1,2)
           hist(coh_ps,100)
        ...
        gamma_change_rms=sqrt(sum((coh_ps-coh_ps_save).^2,"omitnan")/n_ps); % DR nan
...
stamps_save(pmname,ph_patch,K_ps,C_ps,coh_ps,N_opt,ph_res,step_number, ...
    ph_grid,n_trial_wraps,grid_ij,grid_size,low_pass,i_loop,ph_weight, ...
    Nr,Nr_max_nz_ix,coh_bins,coh_ps_save,gamma_change_save) 
