function [] = ps_weed(all_da_flag, no_weed_adjacent, no_weed_noisy)
    ...
    %% Load hardcoded pixels [DR]

    if isfile("../input_azrg")
        forced_ij = load("../input_azrg");
        fprintf("PS_WEED: loaded %i hardcoded pixels from `input_azrg` [DR]\n", size(forced_ij));
    else
        forced_ij = zeros(0,2);
        fprintf("PS_WEED: no hardcoded pixels loaded [DR]\n");
    end
    ...
    forced_ij_idx = find(ismember(ps.ij(:, 2:3), forced_ij, 'rows'));
    forced_ix = ps.ij(forced_ij_idx, 1);

    if isfield(sl, 'keep_ix')
        
        forced_ix2 = find(ismember(sl.ix, forced_ix, 'rows'));
    ...
    disp(find(ismember(ij2(:, 2:3), forced_ij, 'rows'))');
    ...
    if n_ps ~= 0
        if no_weed_noisy == 0
            ...
            if ~strcmpi(small_baseline_flag, 'y')
                ...
                ifg_var = var(dph_noise2, 0, 1, 'omitnan');
    ...
    %% Remove hard coded points from ix_weed [DR]

    forced_ij_mask = ismember(ij2(:,2:3), forced_ij, 'rows');
    n_remain = sum(ix_weed(forced_ij_mask));  
    fprintf("PS_WEED: %i fixed locations remaining\n", n_remain);

    forced_ij_mask = ismember(ij2(:,2:3), forced_ij, 'rows');
    ix_weed(forced_ij_mask) = 1;
    n_remain = sum(ix_weed(forced_ij_mask));  
    fprintf("PS_WEED: %i fixed locations remaining\n", n_remain);
    ...
    fprintf("PS_WEED: %i forced points\n", size(find(ismember(ij2(:, 2:3), forced_ij, 'rows')),1));
    ...
    fprintf("PS_WEED: %i forced points\n", size(find(ismember(ps.ij(:, 2:3), forced_ij, 'rows')),1));

    psname = ['ps', num2str(psver + 1)];
    save(psname, '-struct', 'ps');
    clear ps xy2 ij2 lnlat2;
