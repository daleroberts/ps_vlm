...
void cfloatswap( complex<double>* f )
{
  char* b = reinterpret_cast<char*>(f);
  complex<double> f2;
  char* b2 = reinterpret_cast<char*>(&f2);
...
//  char *meanoutname;
  ...
  cout << "prec: " << prec << " byteswap: " << byteswap << "\n";
...
//  char *maskfilename;
  ...
  for (int i=0; i<num_files; ++i) 
  {
    ...
    cout << ampfilename << " mean: " << calib_factor[i] << "\n";
  ...
  //complex<double>* buffer = new complex<double>[num_files*width]; // used to store 1 line of all amp files
  char* buffer = new char[num_files*patch_linebytes]; // used to store 1 line of all amp files
  complex<double>* bufferf = reinterpret_cast<complex<double>*>(buffer); 
  complex<short>* buffers = reinterpret_cast<complex<short>*>(buffer);


  char* maskline = new char[patch_width];
  for (int x=0; x<patch_width; x++) // for each pixel in range
  {
      maskline[x] = 0;
  }

  char* masterampline = new char[patch_linebytes]; // used to store 1 line of all amp files
  complex<double>* masterlinef = reinterpret_cast<complex<double>*>(masterampline); 
  ...
  while (! ampfile[1].eof() && y < patch_lines) 
  {
     if (y >=0) // was (y >= az_start-1)
       {
       for (int x=0; x<patch_width; x++) // for each pixel in range (width of the patch)
       {
        ...
        complex<double> master_amp; //  master amp value
        if (prec[0]=='s')
        {
            if (byteswap == 1)
            {
            ...
            master_amp=complex<double>(static_cast<double>(masterlines[x].real()), static_cast<double>(masterlines[x].imag()));
        }
        else
        {
        ...
        //if (abs(master_amp)>=0.01)
        //	cout << "y: " << y << " x: " << x << " master_amp: " << abs(master_amp)  << " " << master_amp << endl;
        ...
        for (int i=0; i<num_files; i++)        // for each amp file
	   {
           complex<double> camp; //  amp value
           ...
           if (prec[0]=='s')
           {
               ...
               camp=complex<double>(static_cast<double>(buffers[i*patch_width+x].real()), static_cast<double>(buffers[i*patch_width+x].imag()));
           }
           else
           {
	   ...
	   if (abs(camp) >= 0.01 and abs(camp) != 1) {
           	cout << "camp: " << camp << " calib " << calib_factor[i] << " master " << abs(master_amp) << endl ;
	   }
         
           float amp=abs(camp)/calib_factor[i]/abs(master_amp); // get amp value
           //cout << "amp: " << amp << endl ;
      ...
      //Amplitude disperion^2 
	    float D_sq=num_files*sumampsq/(sumamp*sumamp) - 1; // var/mean^2
