...
void cfloatswap( complex<float64_t>* f )
{
  char* b = reinterpret_cast<char*>(f);
  complex<float64_t> f2;
  char* b2 = reinterpret_cast<char*>(&f2);
  b2[0] = b[3];
  b2[1] = b[2];
  b2[2] = b[1];
...
try {
  ...
  {	  
     ...
     cout << "  parmfile.in(input) SLC file names (complex float64_t)" << endl;
  ...
  complex<float64_t>* buffer = new complex<float64_t>[width];
  char* buffers = reinterpret_cast<char*>(buffer);
  int linebytes;

  if (prec[0]=='s')
  {
     linebytes = sizeof(complex<int32_t>)*width;
  } else {
     linebytes = sizeof(complex<float64_t>)*width;
  }
  ...
  while (! ampfiles.eof() ) // loop over SLC names
  {
    ...
    while (! ampfile.eof() ) // loop to read all file using buffers
    {
      ...
      { 
         ...
         amp_pixel=abs(camp);
         if (amp_pixel >0.001 && maskline[j]==0 && (! isinf(amp_pixel)))       //rejects pixels with low amplitude ~0
         {
          sumamp+=abs(camp);
          nof_pixels++;
