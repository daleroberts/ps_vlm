...
//  char *maskfilename;
  ...
  double D_thresh_sq = D_thresh*D_thresh;
  if (D_thresh<0) { 
      pick_higher=1;
  }
  ...
  double* calib_factor = new double[num_files];
      
  for (int i=0; i<num_files; ++i) 
  {
    parmfile >> ampfilename >> calib_factor[i];
    cout << ampfilename << " " << calib_factor[i];
  ...
  numlines=size/width/sizeof(double)/2;
...
//  complex<double>* buffer = new complex<double>[num_files*width*100]; // used to store 100 lines for all amp files
     ...
     if (y >= az_start-1)
     {
      for(int j=0; j<LineInBuffer; j++) //for each line in azimuth within buffer
      {
       for (int x=rg_start-1; x<rg_end; x++) // for each pixel in range
       {
        ...
        double sumamp = 0;
        double sumampdiffsq = 0;
        int i;

        for (i=0; i<num_files/2; i++)        // for each amp file
	{

           complex<double> camp1, camp2; // get amp value
           if (prec[0]=='s')
           {
               if (byteswap == 1)
               {
	       ...
	       complex<short> cs1 = buffers[(i*2)*width*LineInBuffer+j*width+x];
	       complex<short> cs2 = buffers[(i*2+1)*width*LineInBuffer+j*width+x];
               camp1=complex<double>(static_cast<double>(cs1.real()), static_cast<double>(cs1.real()));
               camp2=complex<double>(static_cast<double>(cs2.real()), static_cast<double>(cs2.real()));
           }
           else
           {
           ...
           double amp1=abs(camp1)/calib_factor[i*2]; // get amp value
           double amp2=abs(camp2)/calib_factor[i*2+1]; // get amp value
...
//           float amp2=abs(buffer[(i*2+1)*width*LineInBuffer+j*width+x])/calib_factor[i*2+1];
	...
	// HERE AN UPDATE NEEDS TO BE INCLUDED WRT the default SVN version
        ...
        meanoutfile.write(reinterpret_cast<char*>(&sumamp),sizeof(double));	

        if (maskline[j*width+x]==0 && sumamp > 0)
        { 
	    double D_a=sqrt(sumampdiffsq/(num_files/2))/(sumamp/num_files); 
