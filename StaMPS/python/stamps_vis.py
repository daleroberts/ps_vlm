...
from typing import no_type_check, List, Tuple, Sequence
...
def lonlat_to_xy(lon: float, lat: float, zoom:int=10) -> Tuple[int, int]:
    ...
    y_tile = int(n * (1 - (np.log(np.tan(np.radians(lat)) + 1 / np.cos(np.radians(lat))) / np.pi)) / 2)
...
def fig_ps_phase_vs_prev_phase(patch:str) -> None:
    ...
    parts = ax.violinplot(np.abs(diff), showmeans=False, showmedians=False, showextrema=False, widths=1.0)
...
def fig_ps_phase_vs_master(patch:str) -> None:
    ...
    for i in range(m):
        for j in range(n):
            k = i*m+j
            ax[i,j].hist2d(np.real(ph[:,i]), np.imag(ph[:,k]), bins=30, cmap='bone_r', range=[[x_min, x_max], [y_min, y_max]])
...
def fig_bperp(patch:str) -> None:
    ...
    ax[0].axhline(y=mean_bperp, color='magenta', linestyle='--', label=f"Mean bperp: {mean_bperp:.2f}", linewidth=1.0)

    # Plot lines for mean +/- 1 std
    ax[0].axhline(y=mean_bperp - std_bperp, color='magenta', linestyle='--', label=f"Mean - 1 std: {mean_bperp - std_bperp:.2f}", linewidth=0.5)
    ax[0].axhline(y=mean_bperp + std_bperp, color='magenta', linestyle='--', label=f"Mean + 1 std: {mean_bperp + std_bperp:.2f}", linewidth=0.5)
    # Plot labels
    ax[0].legend()
...
def fig_incidence_angles(patch:str) -> None:
    ...
    for r, bar in zip(radii, bars):
        bar.set_facecolor(plt.get_cmap("Spectral")(r / 10.))
        bar.set_alpha(0.8)
